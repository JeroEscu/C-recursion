...
//Se crea la función ImprimirSecuencia con cinco parámetros de tipo entero (fila, contador, final, finalP y finalP) para imprimir recursivamente el diseño
int ImprimirSecuencia( int fila, int contador, int final, int finalP )
{
    if ( fila <= 7 ){ 
        ImprimirEspacios( contador, final );
        ImprimirP( contador, finalP );
        SaltoDeLinea();
        ImprimirSecuencia( ( fila + 1 ), contador, ( final + 1 ), ( finalP - 2 ) );
    }//fin del if
...
int main()
{
    //Se llama a la función ImprimirSecuencia con los valores iniciales de la fila = 1, el contador = 1, el final = 34 y la cantidad de letras = 13 
    ImprimirSecuencia( 1, 1, 34, 13 );
    return 0;
}

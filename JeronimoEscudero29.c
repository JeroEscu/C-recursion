...
//Se crea la función SerieEuler 
float SerieEuler( int n, float x )
{
    if( n == 0 ){
        return 1;
    }//de lo contrario
    else{
        return ( pow( x, n ) / Factorial( n ) ) + SerieEuler( ( n - 1 ), x );
    }//fin del if
...
int main()
{
    ...
    //Se llama a la función SerieEuler con los valores de n - 1 (debido a que la serie termina en n = 0) y x como parámetros
    printf( "El valor de e^%f es igual a: %f", x, SerieEuler( n - 1, x ) );

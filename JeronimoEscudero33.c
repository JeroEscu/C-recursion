...
//Se crea la función SerieLogaritmoNatural 
float SerieLogaritmoNatural( int n, float x )
{
    if( n == 0 ){
        return x - 1;
    }//de lo contrario
    else{
        return( ( pow( (-1), n ) * ( pow( ( x - 1 ), ( n + 1 ) ) / ( n + 1 ) ) ) + SerieLogaritmoNatural( ( n - 1 ), x ) );
    }//fin del if
...
int main()
{
    ...
    //Se llama a la función SerieLogaritmoNatural con los valores de n - 1 (debido a que la serie termina en n = 0) y x como parámetros
    printf( "El valor de ln(%f) es igual a: %f", x, SerieLogaritmoNatural( n - 1, x ) );

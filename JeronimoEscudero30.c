...
//Se crea la función SerieCoseno 
float SerieCoseno( int n, float x )
{
    if( n == 0 ){
        return 1;
    }//de lo contrario
    else{
        return( ( pow( (-1), n ) * pow( x, ( 2 * n ) ) ) / Factorial( 2 * n ) + SerieCoseno( ( n - 1 ), x ) );
    }//fin del if
...
int main()
{
    ...
    //Se llama a la función SerieCoseno con los valores de n - 1 (debido a que la serie termina en n = 0) y x como parámetros
    printf( "El valor de cos(%f) es igual a: %f", x, SerieCoseno( n - 1, x ) );
